...
from Source.prediction.predictionPipline import PredictPipline
...
def predict():
    if request.method == "GET":
        ...
        pred_df = dataReadingAndCleaningC.convertHoursAndMinutesToIndependantColumns(df=pred_df,
                                                                                     columName="Arrival_Time")
        pred_df = data.isertValueInDuration(pred_df)
        pred_df = dataReadingAndCleaningC.convertDurationToMinutes(pred_df)
        pred_df = data.dropUncessaryColumns(pred_df)
        PredictPipeline1 = PredictPipline()
